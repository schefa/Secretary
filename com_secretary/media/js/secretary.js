    ...
    function readyFn() {
        Secretary.Fields = function (fields) {
                ...
                getFieldObject(id, ext, undefined, function (json) {
                    if (json)
                        addField(
                            json.id,
                            json.hard,
                            Secretary.Helpers.htmlEntities(json.title),
                            json.box,
                            json.description,
                        );
                });
            ...
            function getFieldObject(id, extension, standard, callback) {
                ...
                if (typeof standard !== "undefined") {
                    input.standard = encodeURIComponent(standard.replace(/#/g, ""));
                }
        ...
        Secretary.Ajax = {
            call: function (container, task, id) {
                ...
                $.getJSON("index.php?option=com_secretary&task=" + task + "&id=" + id, function (data) {
        ...
        Secretary.Search = {
            drawBlockInput: function (container, title) {
                ...
                $(container)
                    .parent()
                    .prepend('<div class="input-blocked">' + title + Secretary.Search.removeInput + "</div>");
                $(container).hide();
            },
            ...
            drawBudgetContainer: function (item, container) {
                ...
                $(".budget").append(
                    '<div class="budget-total">' +
                        ...
                        item.currency +
                        "</div>" +
                        '<div class="budget-category"><a rel="{size: {x: 800, y: 500}, handler:\'iframe\'}" href="index.php?option=com_secretary&view=document&id=' +
                        item.id +
                        '&tmpl=component&layout=preview" class="modal" target="_blank">' +
                        item.category +
                        " / " +
                        item.created +
                        "</a></div>",
        ...
        $(document).on("focus", "input.search-subject-zip, input.search-subject-location", function () {
            ...
            $(this)
                ...
                .autocomplete("instance")._renderItem = function (ul, item) {
                return $("<li>")
                    .append('<a><span class="ui-menuitem-value">' + item.zip + " " + item.location + "</span></a>")
                    .appendTo(ul);
            };
        });
                ...
                results: function () {
                    ...
                    for (var key in featuresList) {
                        if (featuresList.hasOwnProperty(key)) {
                            ...
                            var li =
                                '<div class="added-post clearfix">' +
                                '<div class="added-post-left">' +
                                '<div class="added-post-title">' +
                                subject.firstname +
                                " " +
                                subject.lastname +
                                Secretary.Features.input("id", "hidden", subject.id, counter) +
                                "</div>";
                            delete subject.id;
                            delete subject.firstname;
                            delete subject.lastname;
            ...
            $(".search-features")
                .autocomplete({
                    source: function (request, response) {
                        $.getJSON(
                            "index.php?option=com_secretary&task=ajax.search&section=subjects&source=" + source,
                            {
                    ...
                    select: function (event, ui) {
                        var li =
                            ...
                            '<div class="added-post-title">' +
                            ui.item.value +
                            Secretary.Features.input("id", "hidden", ui.item.id, counter) +
                            "</div>" +
        ...
        $("#sidebar-angle").click(function () {
            ...
            if (children.hasClass(".fa-angle-left")) children.toggleClass("fa-angle-left fa-angle-right");
            else children.toggleClass("fa-angle-right fa-angle-left");

            var angle = $(".secretary-sidebar-container").find(".fa.pull-right");
            if (angle.hasClass(".fa-angle-down")) angle.toggleClass("fa-angle-down fa-angle-right");
            else angle.toggleClass("fa-angle-left fa-angle-right");
        ...
        $("#settings_access .input-small").change(function () {
            ...
            var loader =
                "background: url(../media/system/images/modal/spinner.gif);display:inline-block;width:16px;height:16px;";
        ...
        $("#pdf_select").change(function () {
            ...
            switch (value) {
                case "mpdf":
                    $(".secretary-desc #mpdf").show();
                    break;
                case "dompdf":
                    $(".secretary-desc #dompdf").show();
                    break;
            }
        });
    }
