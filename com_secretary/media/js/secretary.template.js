(function ($) {
    ...
    function readyFn($) {
        ...
        var boxToolbar =
            '<div class="box-toolbar"><div class="box-toolbar-group"><div class="box-mover ui-draggable-handle"><i class="fa fa-arrows"></i></div><div class="box-delete"><i class="fa fa-remove"></i></div></div><div class="box-toolbar-group"><div class="box-label">CSS</div><div class="box-cssclass" contenteditable="true">classname</div></div></div>';
        ...
        $(".add-box").click(function () {
            ...
            switch (type) {
                ...
                case "textarea":
                    var $canvasElement = $(
                        '<div class="ui-draggable ui-draggable-handle secretary-canvas-box" style="position:absolute;top:0px;left:0px;z-index:' +
                            zindex +
                            '"><div class="box-text" contentEditable="true"></div>' +
                            boxToolbar +
                            "</div>",
                    );
        ...
        $(document).on("keyup", ".box-cssclass", function (event) {
        ...
        function setDraggable(element) {
            element.draggable({
                ...
                stop: function (e, ui) {
                    ...
                    if (posLeft + boxWidth > $(canvas).width()) posLeft = $(canvas).width() - boxWidth;

                    var boxHeight = textArea.height();
                    if (posTop + boxHeight > $(canvas).height())
                        posTop = $(canvas).height() - boxHeight - offsetToolbar;

                    if (posTop < 0 && posLeft < 0) $(this).css({ left: 0, top: 0 });
                    else if (posTop < 0) $(this).css({ left: posLeft, top: 0 });
                    else if (posLeft < 0) $(this).css({ left: 0, top: posTop });
