    ...
    Secretary.Charts = function (output) {
        ...
        if (typeof arguments[1].series !== "undefined") {
            ...
            function createChart() {
                ...
                if (settings.yScaleFromZero === false) settings.maxValue = settings.maxValue - settings.minValue;

                var totalColumns =
                    settings.labels !== null && settings.series.length < settings.labels.length
                ...
                settings.steps = parseFloat(
                    ((settings.height - settings.svgPaddingBottom) / settings.maxValue) * settings.shortener,
                );

                var adjustedRectWidth =
                    (settings.svgWidth - settings.svgPaddingLeft) / (totalColumns * maxValueArray[2] * 1.1) - 6;

                if (adjustedRectWidth < 3 || settings.rectWidth < 3) {
                    settings.rectWidth = settings.minRectWidth;
                } else {
                    settings.rectWidth =
                        adjustedRectWidth < settings.maxRectWidth ? adjustedRectWidth : settings.maxRectWidth;
                }
                settings.xAxisBarCenter =
                    ((settings.svgWidth - settings.svgPaddingLeft) / totalColumns - settings.rectWidth) / 2;
        ...
        create: {
            ...
            Circle: function (parent, attributes) {
                ...
                for (var key in attributes) {
                    if (attributes.hasOwnProperty(key)) circle.setAttributeNS(null, key, attributes[key]);
                }
            ...
            DashedLine: function (parent, attributes) {
                ...
                if (typeof attributes !== "undefined") {
                    for (var key in attributes) {
                        if (attributes.hasOwnProperty(key)) line.setAttributeNS(null, key, attributes[key]);
                    }
                }
            ...
            Group: function (id, transform) {
                ...
                if (typeof transform !== "undefined") group.setAttribute("transform", transform);
                return group;
            },
            ...
            PieSection: function (parent, section, tooltip) {
                ...
                newSection.setAttributeNS(
                    null,
                    "transform",
                    "rotate(" + section.R + ", " + section.L + ", " + section.L + ")",
                );
        ...
        draw: {
            Bar: function (options) {
                ...
                if (typeof options.data === "object") {
                    for (var el in options.data)
                        if (options.data.hasOwnProperty(el)) sumValues += parseFloat(options.data[el]);
                } else sumValues = options.data;

                fullRectHeight = sumValues * settings.steps;
                if (!settings.yScaleFromZero && settings.minValue < 0)
                    fullRectHeight -= settings.minValue * settings.steps;
                ...
                if (
                    ...
                    options.lastValue = lastValue;
                    options.cssClass =
                        typeof options.className !== "undefined" ? options.className[options.x] : "line-" + options.x;
            ...
            BarStatus: function (options, settings) {
                ...
                var item = Secretary.Charts.SVGFactory.create.Line(options.parent, {
                    x1: rectX,
                    x2: rectX,
                    y1: rectHeight + options.rectPosY,
                    y2: options.rectPosY,
                    class: "line " + options.cssClass,
                    style: "stroke-width:" + settings.rectWidth + "px",
                });
            ...
            Bars: function () {
                ...
                var tooltip =
                    settings.tooltip !== false ? Secretary.Charts.SVGFactory.create.Tooltip(settings.container) : "";
                ...
                for (var barsGroup = 0; barsGroup < settings.series.length; barsGroup++) {
                    if (typeof settings.series[barsGroup] !== "undefined") {
                        ...
                        if (settings.labels !== null && typeof settings.labels[barsGroup] !== "undefined") {
                            if (typeof settings.labels[barsGroup][1] !== "undefined") {
                                options.label = settings.labels[barsGroup][1];
                            } else {
                                options.label = settings.labels[barsGroup].toString();
                            }
                        }
                        options.tip = tooltip;
                        ...
                        if (
                            typeof settings.series[barsGroup] === "number" ||
                            typeof settings.series[barsGroup].length === "undefined"
                        ) {
                            ...
                            while (options.barNr < settings.series[barsGroup].length) {
                                if (typeof settings.series[barsGroup][options.barNr] !== "undefined") {
                                    ...
                                    options.className =
                                        ...
                                        typeof settings.classes[barsGroup][options.barNr] !== "undefined"
                                            ? settings.classes[barsGroup][options.barNr]
                                            : "";
                                    Secretary.Charts.SVGFactory.draw.Bar(options);
                                    options.barNr++;
                                }
            ...
            Graph: function () {
                ...
                for (var barsGroup = 0; barsGroup < settings.series.length; barsGroup++) {
                    if (typeof settings.series[barsGroup] !== "undefined") {
                        // X Position
                        settings.rectPosX =
                            settings.svgPaddingLeft + settings.xAxisBarCenter + settings.rectDistance * barsGroup;

                        var fullRectHeight = 0,
                            rectHeight = 0,
                            rectPosY = 0;

                        if (settings.yScaleFromZero === false) settings.series[barsGroup] -= settings.minValue;
                        ...
                        // Draw
                        Secretary.Charts.SVGFactory.create.Line(group, {
                            class: "line",
                            x1: startX,
                            x2: endX,
                            y1: startY,
                        ...
                        var item = Secretary.Charts.SVGFactory.create.Line(group, {
                            class: "point",
                            x1: startX,
                            x2: startX,
                            y1: startY,
                        ...
                        if (typeof item !== "undefined" && settings.tooltip !== false) {
                            ...
                            if (settings.labels !== null && settings.labels[1].length > 0)
                                item.title +=
                                    '<div class="secretary-charts-tooltip-subtext">' + settings.labels[1] + "</div>";
                            item.title +=
                                '<div class="secretary-charts-tooltip-title">' + settings.series[barsGroup] + "</div>";
                            Secretary.Charts.Utils.Tooltip.attach(item);
                        }
                        endY = startY;
                        endX = startX;
            ...
            Pie: function () {
                ...
                var sum = settings.series.reduce(function (pv, cv) {
                    return pv + cv;
                }, 0);
                ...
                var calcSectors = function (data) {
                    var sectors = [];
                    var l = parseInt(data.width, 10) / 2,
                        a = 0,
                        aRad = 0,
                        z = 0,
                        x = 0,
                ...
                for (var j = 0; j < sectors.length; j++) {
                    ...
                    var label =
                        settings.labels[j].toString().length > 32
                            ? settings.labels[j].toString().substr(0, 32) + "..."
                            : settings.labels[j];
                    titles[j] =
                        '<span class="legend-perc ' +
                        CSSClass +
                        '">' +
                        settings.percs[j] +
                    ...
                    item.title +=
                        '<div class="secretary-charts-tooltip-subtext">' +
                        sectors[j].value +
                        " (" +
                        sectors[j].perc +
                        "%)</div>";
            ...
            Legend: function (options) {
                if (options.legend.series == null) return false;
            ...
            XAxis: function () {
                ...
                Secretary.Charts.SVGFactory.create.Line(xAxisGroup, {
                    class: "axisZero",
                    x1: settings.svgPaddingLeft,
                    x2: settings.svgPaddingLeft,
                    y1: fullHeight,
                    y2: 0,
                    style: settings.style.zeroAxis,
                });
                ...
                for (var i = 0; i < settings.labels.length; i++) {
                    ...
                    Secretary.Charts.SVGFactory.create.Text({
                        parent: group,
                        text: settings.labels[i][0].toString(),
                        x: posX + settings.xAxisBarCenter,
                        y: settings.height - 10,
                    });

                    if (i > 0)
                        Secretary.Charts.SVGFactory.create.DashedLine(group, {
                            class: "dashedline xline" + i,
                            x1: posX,
                            x2: posX,
                            y1: fullHeight,
                            y2: 0,
                            style: settings.style.dashedLine,
                        });
            ...
            YAxis: function () {
                ...
                if (yAxisValue * settings.horzLines < settings.maxValue) yAxisValue *= 2;
                ...
                for (var linesNeeded = 0; linesNeeded < settings.horzLines; linesNeeded++)
                    if (settings.maxValue <= yAxisValue * linesNeeded) break;
                ...
                if (!settings.yScaleFromZero && settings.minValue < 0) {
                    ...
                    Secretary.Charts.SVGFactory.create.Text({
                        parent: group,
                        text: settings.minValue.toString(),
                        x: 0,
                        y: fullHeight,
                    });
                    Secretary.Charts.SVGFactory.create.Line(group, {
                        class: "axisZero yline0",
                        x1: settings.svgPaddingLeft,
                        x2: fullWidth,
                        y1: posY,
                        y2: posY,
                ...
                for (var i = 0; i <= linesNeeded; i++) {
                    ...
                    // Labels
                    if (settings.yScaleFromZero === false) var label = (yAxisValue * i + settings.minValue).toString();
                    else var label = (yAxisValue * i).toString();
    ...
    Secretary.Charts.Utils = {
        Tooltip: {
            ...
            pathMouseMove: function (e) {
                ...
                if (e.clientY + e.target.tip.offsetHeight + 40 > windowHeight) PosY = PosY - e.target.tip.offsetHeight;

                ((e.target.tip.style.top = PosY + "px"), (e.target.tip.style.left = PosX + 10 + "px"));
            },
        },
