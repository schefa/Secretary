...
(function( $, Secretary ) {
	...
	function readyFn() {
		...
		Secretary.Document = {	
			...
			addDocument : function (id, nr, created, deadline, title, subtotal, tax, total, subjectid) {
				...
				function insertRow(subjectName) {
					...
					$('<div class="table-item table-item-document clearfix dd-item" data-id="'+ parseInt(counter)+'">' + rowHtml + '</div>').appendTo('.table-items-list div.dd-list').show();
			...
			add : function (title, desc, pno, quantity, entity, price, taxRate, total) {
					...
					html = html.replace(/##total##/g, total);
					$('<div class="table-item clearfix dd-item" data-id="'+ parseInt(counter)+'">' + html + '</div>').appendTo('.table-items-list div.dd-list').show();
					Secretary.Document.counter.set( parseInt(counter) + 1 );
			...
			calculate : {
				...
				renderTaxBreakdown : function( byRate ) {
					...
					for ( var rate in byRate ) {
						if(byRate.hasOwnProperty(rate)) {
							...
							$('<div class="fullwidth"><div class="pull-right secretary-input-group document-clean-input clearfix"><div class="secretary-input-group-left"><span class="tax-proz">' + rate + '%</span><span class="tax-val">' + parseFloat( amount ).toFixed(2) + '</span><input type="hidden" name="jform[taxtotal]['+ rate +']" value="'+ amount.toFixed(4) +'" /></div><div class="secretary-input-group-right currency-control">'+ currency + '</div></div></div>').appendTo('.document-taxrate');
						}
					}
				},
				...
				total : function() {
					...
					var items = Secretary.Document.calculate.collectItems();
					var totals = Secretary.TaxCalc.computeDocumentTotals(taxtype, items, { amount: rabatt, percent: rabattProz });
		...
		$(document).on("keyup change", '#jform_rabatt', function() { $('input#jform_rabatt_proz').val(""); });
		$(document).on("keyup change", '#jform_rabatt_proz', function() { $('input#jform_rabatt').val(""); });
		...
		$('.table-items-list').nestable({ dragClass : "table-item dd-dragel" , maxDepth : 1, listNodeName : 'div', itemNodeName : 'div' });
