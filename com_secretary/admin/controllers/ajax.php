...
class SecretaryControllerAjax extends \Joomla\CMS\MVC\Controller\FormController
{
	...
	public function update()
	{
		...
		if (!empty($data) && $canDo->get('core.edit'))
		{
			...
			if ($result)
			{
				echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_SAVE_SUCCESS');
			}
            else
			{
				echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_SAVE_FAILED');
			}
		}
		$this->app->close();
	}
	...
	public function saveProgress()
	{
		...
		// The progress bar only exists on tasks; without this check a user with
		// core.edit on 'time' could redirect the UPDATE at any #__secretary_<extension> table.
		if ($extension !== 'tasks')
		{
			$this->app->close();
	...
	public function projectTimer()
	{
		...
		if (isset($action) && isset($itemID) && isset($projectID))
		{
			if ($action == 'stop')
			{
				...
				{
					echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_FAILURE');
				}
			}
	...
	public function checkDocumentNumber()
	{
		...
		if (!empty($nr) && !empty($catid))
		{
            ...
            if (!empty($usedNr))
			{
				...
				$range = range($down, $up);
				echo '<div class="alert alert-warning number-in-use"><span class="tiny-text">' . \Joomla\CMS\Language\Text::_('COM_SECRETARY_ERROR_DOUBLE_NO') . '</span><ul>';
				...
				for ($i = 0; $i < count($usedNr ?? []); $i++)
				{
                    ...
                    if ($range[$i] != $usedNr[$i])
					{
						echo '<span class="no-is-free">' . \Joomla\CMS\Language\Text::sprintf('COM_SECRETARY_THIS_IS_FREE', $range[$i]) . '</span>';
						unset($range[$i]);
	...
	public function updatePermission()
	{
        ...
        if ($user->authorise('core.admin', 'com_secretary'))
		{
            ...
            if (empty($parentAssetId))
			{
				...
				$parentAsset->title = \Joomla\CMS\Language\Text::_('COM_SECRETARY_COMPONENT');
