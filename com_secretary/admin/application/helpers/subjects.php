...
class Subjects
{
	...
	public static function addNewSubject($subject, $timestamp = false, $created_by = false)
	{
		...
		if (empty($subjectID) && !empty($subject[1]))
		{
			...
			// Google Maps
			$coords = (!empty($subject[3])) ? \Secretary\Helpers\Locations::getCoords($street, $zip, $location) : array();
			$profile->lat = (isset($coords['lat'])) ? $coords['lat'] : 0.0;
			$profile->lng = (isset($coords['lng'])) ? $coords['lng'] : 0.0;
	...
	public static function importUsers()
	{
		...
		if (empty($result))
		{
			$msg = \Joomla\CMS\Language\Text::_("COM_SECRETARY_SUBJECTS_IMPORT_ADDED_NAMES") . '<br>' . \Joomla\CMS\Language\Text::_("COM_SECRETARY_SUBJECTS_IMPORT_NOONE");
		}
        else
		{
			$msg = \Joomla\CMS\Language\Text::sprintf("COM_SECRETARY_SUBJECTS_IMPORT_ADDED_NAMES", count($result ?? [])) . '<br>' . implode(", ", $result);
		}
