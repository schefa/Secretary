...
class Items
{
	...
	public static function getField($id = 1, $extension = 'documents', $default = '', $counter = "##counter##")
	{
		...
		$query->select('*')
			...
			->where('( (' . $db->qn('extension') . '=' . $db->quote($extension) . ' OR extension = ' . $db->quote("folders") . ') OR ' . $db->qn('extension') . '=' . $db->quote("system") . ')')
			->order('id');
		$db->setQuery($query);
		$field = $db->loadObject();
		...
		if (!empty($field))
		{
			...
			$return->title = \Secretary\Utilities::cleaner(\Joomla\CMS\Language\Text::_($field->title), true);

			$return->description = \Joomla\CMS\Language\Text::_(trim($field->description));
        ...
        switch ($object->type)
		{
			case 'list':
				if ($fieldvalues = json_decode($object->values, true))
				{
					...
					{
						$options[] = \Joomla\CMS\HTML\HTMLHelper::_('select.option', $key, \Joomla\CMS\Language\Text::_($val));
					}
				}
				$html = \Joomla\CMS\HTML\HTMLHelper::_('select.genericlist', $options, $name, '', 'value', 'text', \Secretary\Utilities::cleaner($standard));
				break;
			case 'sql':
                ...
                if ($canManage == 1)
				{
					...
					if (empty($items))
					{
						$options = array(0 => \Joomla\CMS\Language\Text::_('COM_SECRETARY_NONE'));
                    }
                    else
					{
                        foreach ($items as $item)
						{
							$options[] = \Joomla\CMS\HTML\HTMLHelper::_('select.option', $item->id, \Joomla\CMS\Language\Text::_($item->title));
                        }
                    }
			...
			case 'search':
                ...
                if ($standard > 0)
				{
					$searchValue = \Joomla\CMS\Language\Text::_('COM_SECRETARY_' . $table);
					$title = \Secretary\Database::getQuery($table, $standard, 'id', 'title', 'loadResult');
					$html = '<input class="search-' . $table . '" type="text" value="' . $title . '" placeholder="' . \Joomla\CMS\Language\Text::sprintf('COM_SECRETARY_SEARCH_THIS', $searchValue) . '" />';
				}
                else
				{
			...
			default:
				...
				{
					$placeholder = ' placeholder="' . \Joomla\CMS\Language\Text::_('COM_SECRETARY_FIELD_VALUE') . '"';
				}
	...
	public static function getFields($extension, $excludes = array())
	{
		...
		$query->where(' ( (' . $db->qn('extension') . ' = ' . $db->quote($extension) . ' OR extension = ' . $db->quote("folders") . ') OR ' . $db->qn('extension') . '=' . $db->quote("system") . ')');
	...
	{
		// Return the same empty shape as the else branch below rather than null -
		// callers index straight into ['fields'] and ['count'].
		if (empty($data))
		{
			return array('count' => 0, 'fields' => '');
		}
		...
		if ($fields = json_decode($data, true))
		{
			...
			foreach ($fields as $key => $field)
			{
				if (isset($fields[$key][1]) && isset($fields[$key][2]) && isset($fields[$key][3]))
				{
					...
					$fields[$key][1] = \Joomla\CMS\Language\Text::_(\Secretary\Utilities::cleaner($fields[$key][1], true));
