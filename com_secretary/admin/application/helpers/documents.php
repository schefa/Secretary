...
class Documents
{
	...
	public static function acquit($pk)
	{
		if (!empty($pk))
		{
			...
			if ($canChange)
			{
				...
				$title = $item->created;
				\Secretary\Joomla::getApplication()->enqueueMessage(\Joomla\CMS\Language\Text::sprintf('COM_SECRETARY_PAIDUP_THAT_DOCUMENT', $title));
			}
            else
			{
				throw new \RuntimeException(\Joomla\CMS\Language\Text::_('COM_SECRETARY_PERMISSION_FAILED'), 1);
			}
		}
	...
	public static function getDocumentsPrepareRow(&$row, &$result)
	{
		...
		{
			$row["id"] = $result->id;
			$row["category"] = isset($result->category) ? \Joomla\CMS\Language\Text::_($result->category) : '';
			...
			{
				$row["title"] = \Joomla\CMS\Language\Text::_('COM_SECRETARY_DOCUMENT');
			}
			...
			{
				$model = \Joomla\CMS\MVC\Model\BaseDatabaseModel::getInstance('Subject', 'SecretaryModel');
