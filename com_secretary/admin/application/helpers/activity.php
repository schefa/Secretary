...
class Activity
{
	...
	public static function get($id)
	{
        ...
        if (empty($userName))
		{
			$userName = \Joomla\CMS\Language\Text::_('COM_SECRETARY_UNKNOWN');
		}
		...
		$folder = new \Joomla\CMS\Object\CMSObject;
		$itemTitle = '';
		...
		if ($activity->extension === 'documents')
		{
			...
			$itemTitle = (!empty($folder->alias)) ? \Joomla\CMS\Language\Text::_($folder->alias) : \Joomla\CMS\Language\Text::_('COM_SECRETARY_DOCUMENT');
		}
        else
		{
			$itemTitle = \Joomla\CMS\Language\Text::_('COM_SECRETARY_' . strtoupper($view));
		}
		...
		{
			$linkItem = '<a href="' . \Secretary\Route::create('time', array('id' => $activity->itemID, 'extension' => 'tasks')) . '">' . \Joomla\CMS\Language\Text::_('COM_SECRETARY_TASK') . '</a>';
		}

		// Result
		$result = $linkItem . ' ' . \Joomla\CMS\Language\Text::_('COM_SECRETARY_ACTIVITY_' . strtoupper($activity->action)) . ' ' . \Joomla\CMS\Language\Text::_('COM_SECRETARY_BY') . ' ' . $userName;
