...
class Times
{
	...
	public static function getProjectStatus($itemID, $projectID)
	{
		...
		if (empty($status))
		{
			$status = new \Joomla\CMS\Object\CMSObject;
			$status->id = 0;
			$status->action = 'stop';
			$status->created = NULL;
		}
	...
	public static function getMonthname($id = null)
	{
        ...
        else
		{
            ...
            for ($m = 1; $m <= 12; $m++)
			{
				...
				$result[] = \Joomla\CMS\HTML\HTMLHelper::_('select.option', $m, $month);
            }
		}
	...
	public static function getWeekDayname($id = null)
	{
        ...
        else
		{
			...
			{
				$result[] = \Joomla\CMS\HTML\HTMLHelper::_('select.option', $key, \Joomla\CMS\Language\Text::_($value));
            }
		}
	...
	public static function getTimesToolbar($task)
	{
        ...
        for ($x = 0; $x < count($array ?? []); $x++)
		{
			...
			$result .= '<a href="' . \Joomla\CMS\Router\Route::_($array[$x]['link']) . '" class="btn btn-default ' . $active . '">' . \Joomla\CMS\Language\Text::_($array[$x]['title']) . '</a>';
		}
	...
	public static function saveRepetition($extension, $id, $created, $repetition, $zyklus = false)
	{
        ...
        if (!empty($repetition))
		{
			$endTime = strtotime('+' . ($repetition['zyklus'] * $repetition['endTime']) . ' ' . $repetition['type'], $startTime);
		}
		...
		if ($saved->time_id > 0)
		{
            ...
            if (!empty($endTime))
			{
				$sql->set('endTime = ' . $db->escape($endTime));
			}
	...
	{
		// Naive Loesung
		\Joomla\CMS\Table\Table::addIncludePath(SECRETARY_ADMIN_PATH . '/models/tables');
		$section = \Secretary\Application::getSingularSection($extension);
		$table = \Joomla\CMS\Table\Table::getInstance($section, "SecretaryTable");
		...
		foreach ($results as $result)
		{
            ...
            if ($extension == 'times')
			{
				$nextEndTime = strtotime($result->int_in_words, strtotime($result->endDate));
			}
		...
		if (empty($msgResult))
		{
			$msg = \Joomla\CMS\Language\Text::sprintf("COM_SECRETARY_REPETITION_SAVED", 0) . '<br>' . \Joomla\CMS\Language\Text::_("COM_SECRETARY_REPETITION_NON");
		}
        else
		{
			$msg = \Joomla\CMS\Language\Text::sprintf("COM_SECRETARY_REPETITION_SAVED", count($msgResult ?? [])) . '<br>' . implode("<br>", $msgResult);
		}
	...
	public static function getTimeColor($fieldss)
	{
        ...
        if ($fields = json_decode($fieldss))
		{
			...
			{
				if (is_array($field) && isset($field[3]) && ($field[3] == 'timeColor' || $field[3] == 'color'))
				{
	...
	public static function subscription()
	{
        ...
        if (empty($extension))
		{
			return \Joomla\CMS\Language\Text::_('COM_SECRETARY_NOT_FOUND') . ': ' . \Joomla\CMS\Language\Text::_('COM_SECRETARY_EVENT');
		}
		...
		$event = \Joomla\CMS\Table\Table::getInstance($section, "SecretaryTable");
        ...
        if (!empty($cid))
		{
            ...
            if (empty($contact->id))
			{
				return \Joomla\CMS\Language\Text::_('COM_SECRETARY_NOT_FOUND') . ': ' . \Joomla\CMS\Language\Text::_('COM_SECRETARY_SUBJECT');
            }
		}
        else
		{
			...
			{
				return \Joomla\CMS\Language\Text::_('COM_SECRETARY_EMAIL_INVALID');
            }

			// Data
			$data = array('id' => 0, 'business' => $event->business, 'firstname' => $firstname, 'lastname' => $lastname, 'email' => $contactEmail);

			$subjectTable = \Joomla\CMS\Table\Table::getInstance("Subject", "SecretaryTable");

			$subjectTable->prepareStore($data);
			
            if (!$subjectTable->bind($data))
			{
                return $subjectTable->getError();
            }
			
            if (!$subjectTable->check())
			{
                        return $subjectTable->getError();
            }
			
            if (!$subjectTable->store($data))
			{
                        return $subjectTable->getError();
            }
		...
		return \Joomla\CMS\Language\Text::_('COM_SECRETARY_TIME_SUBSCRIPTION_SUCCESS');
	}
