...
class Uploads
{
	...
	/**
	 * Resolves a path under the uploads root and confirms it doesn't escape it - defends
	 * against path traversal if a business/folder/title value is ever crafted or corrupted.
	 * Shared by every read/delete site that builds an uploads path from a DB row.
	 *
	 * @param string $relativePath path relative to the uploads root, e.g. "12/emails/invoice.pdf"
	 * @return string|false absolute, contained path, or false if it escapes the uploads root or doesn't exist
	 */
	public static function safeUploadPath($relativePath)
	{
		$base = realpath(SECRETARY_ADMIN_PATH . '/uploads');
		$target = realpath(SECRETARY_ADMIN_PATH . '/uploads/' . $relativePath);
	...
	public static function getUploadFile($item, $class = NULL, $size = NULL, $return = FALSE)
	{
		...
		$file = self::safeUploadPath($item->business . '/' . $item->folder . '/' . $item->title);
	...
	public static function upload($folder, $extension, $upload_title, $newID = FALSE, $files = array())
	{
        ...
        if (isset($checkBox))
		{
			$uploadObj = \Joomla\CMS\Table\Table::getInstance("Uploads", "SecretaryTable");
	...
	private static function uploadDocument($files, $itemID = FALSE, $folder = 'document', $extension = '')
	{
		...
		{
			$app->enqueueMessage(\Joomla\CMS\Language\Text::sprintf('COM_SECRETARY_DOCUMENT_INVALID_SIZE', \Secretary\Utilities\Number::human_filesize($allowedSize)), 'error');
		...
		{
			$app->enqueueMessage(\Joomla\CMS\Language\Text::sprintf('COM_SECRETARY_DOCUMENT_INVALID_EXTENSION', $allowedExt), 'error');
		...
		$safeName = basename($files['name']);
		$fileNamePathName = (!empty($itemID)) ? ($itemID . '_' . $safeName) : (date('Y-m-d') . '_' . $safeName);
		$uploadPath = SECRETARY_ADMIN_PATH . '/uploads/' . $business['id'] . '/' . $folder . '/' . $fileNamePathName;

		// Existiert in der UploadRoutine
		if (isset($files['uploaded']) && strlen($files['uploaded']) > 0 && File::copy($files['uploaded'], $uploadPath))
		{
			self::_updateTables($extension, $itemID, $folder, $fileNamePathName, $business['id']);
			
            return $uploadPath;
		}
        elseif (File::upload($files['tmp_name'], $uploadPath))
		{
			$app->enqueueMessage(\Joomla\CMS\Language\Text::sprintf('COM_SECRETARY_UPLOAD_SUCCESSFUL', $files['name']), 'notice');
