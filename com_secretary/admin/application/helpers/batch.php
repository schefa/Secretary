	...
	protected function batchCopy($value, $pks, $contexts)
	{
		...
		if ($parentId)
		{
            ...
            if (!$canCreate)
			{
				...
				$this->setError(\Joomla\CMS\Language\Text::_('COM_SECRETARY_CATEGORIES_BATCH_CANNOT_CREATE'));
		...
		if (empty($parentId))
		{
			...
			{
				// DatabaseDriver::getErrorMsg() was removed in Joomla 4; the failure
				// is reported by the table itself.
		...
		if ($parentId)
		{
			...
			foreach ($pks as $pk)
			{
				if (!$user->authorise('core.edit', $extension . '.folder.' . $pk))
				{
					...
					$this->setError(\Joomla\CMS\Language\Text::_('COM_SECRETARY_CATEGORIES_BATCH_CANNOT_EDIT'));
