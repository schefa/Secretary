...
class Times
{
    ...
    public static function monthTable($month, $year, $setitems = false)
    {
        ...
        $calendar .= '<tr class="calendar-row"><td class="calendar-day-head" colspan="7"><span>' . \Joomla\CMS\Language\Text::_(\Secretary\Helpers\Times::getMonthname($month)) . '</span></td></tr>';
        ...
        for ($list_day = 1; $list_day <= $items['days_in_month']; $list_day++)
        {
            ...
            $calendar .= '<a href="' . \Joomla\CMS\Router\Route::_('index.php?option=com_secretary&amp;view=times&amp;section=day&amp;month=' . $month . '&amp;day=' . $list_day) . '">' . $list_day . '</a>';
            $calendar .= '</div>';
    ...
    public static function listViewProject($i, $item, $vorspannSecs, $intervall)
    {
        ...
        <div class="secretary-row-main secretary-row-inner clearfix">
            ...
            <div class="checkbox center hidden-phone" data-extension="<?php echo $item->extension; ?>">
            ...
            <div class="title">
                <div class="secretary-time-title">
                    <a href="<?php echo \Joomla\CMS\Router\Route::_('index.php?option=com_secretary&view=time&extension=' . $item->extension . '&id=' . (int) $item->time_id); ?>"><?php echo $item->time_title; ?></a>
                </div>

                <div class="time-btn-group-white clearfix">
                    <div class="btn-group-col"><?php echo \Joomla\CMS\HTML\HTMLHelper::date($item->startDate, 'd.m.Y'); ?></div>
                    <div class="btn-group-col"><?php echo \Joomla\CMS\HTML\HTMLHelper::date($item->startDate, 'h:i'); ?></div>
                    <?php 
                    if ($item->endDate > 0)
                    {
                        ?>
                        <div class="time-btn-group-white-sep">&#8212;</div>
                        <div class="btn-group-col"><?php echo \Joomla\CMS\HTML\HTMLHelper::date($item->endDate, 'd.m.Y'); ?></div>
                        <div class="btn-group-col"><?php echo \Joomla\CMS\HTML\HTMLHelper::date($item->endDate, 'h:i'); ?></div>
                    <?php } ?>
                </div>
                ...
                {
                    ?>
                    <div class="times-list-attendees"><span><?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_TIME_CONTACTS'); ?>:</span><?php echo $item->contactsNames; ?></div>
                <?php } ?>
            </div>
            ...
            <div class="time-calender-td">
                <div class="time-calender-container">
                    ...
                    if ($nextTimeStart > 0)
                    {
                        ...
                        ?>
                        <div date-title="<?php echo  date('d.m.Y', $nextTimeStart); ?>" class="hasTooltip start-position" style="margin-left:<?php echo $startPos ?>%<?php echo $widthPos;
                                                                                                                                                                    echo $item->timeColor; ?>"></div>
                    <?php } ?>
                </div>
            </div>
    ...
    public static function listViewProjectTaskRow($x, $task, $userId, $prev, $intervall)
    {
        if (!empty($task->task_id) && !empty($task->time_id))
		{
            ...
            if ($contacts = json_decode($task->contacts))
			{
				...
				{
                    if (is_object($contact) && ($userId > 0) && intval($contact->id) === $userId)
                    {
                    	$teamMember = $userId;
                    }
                }
            ...
            <div class="fullwidth secretary-row-inner project-task-item row<?php echo $x % 2; ?> ">
                ...
                <div class="title">
                    ...
                    <span class="project-task-title">
                        <a href="<?php echo \Joomla\CMS\Router\Route::_('index.php?option=com_secretary&view=time&extension=tasks&id=' . (int) $task->task_id); ?>">
                    ...
                    <div class="btn-group pull-right projectTimer-container">
                        ...
                        <?php if ($teamMember != 0)
                        {
                            ...
                            <div class="btn projectTimer" data-item="<?php echo $task->task_id; ?>" data-project="<?php echo $task->time_id; ?>">
                                <a <?php 
                                if ($taskprojectTimer->action == "start")
                                {
                                    echo 'style="display:none;"';
                                } ?> data-task="start">
                                    <i class="fa fa-play"></i>
                                </a>
                                <a <?php if ($taskprojectTimer->action != 'start')
                                {
                                    echo 'style="display:none;"';
                                   } ?> data-task="stop">
        ...
        if (!empty($task->id) && !empty($task->projectID))
		{
            ...
            <div class="project-task-item clearfix">
                ...
                <div class="pull-left project-task-item-title">
                    <?php echo str_repeat("&#8212; ", $task->level); ?>
                    <span class="project-task-title">
                        <a href="<?php echo \Joomla\CMS\Router\Route::_('index.php?option=com_secretary&view=time&extension=tasks&id=' . (int) $task->id); ?>">
                            <?php echo $task->title; ?>
                        </a>
                    </span>
                </div>
