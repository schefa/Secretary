...
class Google
{
    ...
    public static function maps($items, $type = 'contacts')
    {
        ...
        // Without a key Google now rejects the request outright (ApiProjectMapError/
        // NoApiKeys in the console) rather than serving a degraded free map, so there's
        // nothing usable to render - show a configuration hint instead of a broken map.
        if (strlen($key) === 0)
		{
            return '<div class="alert alert-warning">' . \Joomla\CMS\Language\Text::_('COM_SECRETARY_GMAPS_MISSING_KEY') . '</div>';
        }
...
(function($) {  
    ...
    window.initMap = function() {
        ...
        $html[] = '</script><script async defer src="https://maps.google.com/maps/api/js?' . $keyString . 'loading=async&amp;callback=initMap"></script>';
