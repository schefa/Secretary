...
class Configuration
{
    ...
    public static function getRulesField($section, $assetId = NULL)
    {
        ...
        // Prepare output
        $html = array();
        $html[] = '<div class="secretary-desc margin-bottom">' . \Joomla\CMS\Language\Text::_('JLIB_RULES_SETTINGS_DESC') . '</div>';
        ...
        foreach ($groups as $group)
		{
            ...
            // Bootstrap 5 carries the active state on the link, not the item.
            $html[] = '<li class="nav-item">';
            $html[] = '<a class="nav-link ' . $active . '" href="#permission-' . $section . $group->value . '" data-bs-toggle="tab">';
        ...
        foreach ($groups as $group)
		{
            ...
            if (!empty($actions))
            {
                ...
                foreach ($actions as $action)
                {
                    ...
                    $inheritedRule = \Joomla\CMS\Access\Access::checkGroup($group->value, $action->name, $assetId);
                    ...
                    $html[] =    '<option value="0"' . ($assetRule === false ? ' selected="selected"' : '') . '>' . \Joomla\CMS\Language\Text::_('JLIB_RULES_DENIED') . '</option>';
                    $html[] =    '<option value="1"' . ($assetRule === true ? ' selected="selected"' : '') . '>' . \Joomla\CMS\Language\Text::_('JLIB_RULES_ALLOWED') . '</option>';
                    $html[] = '</select>&#160;';
                    ...
                    if ($canCalculateSettings)
					{
                        ...
                        if (\Joomla\CMS\Access\Access::checkGroup($group->value, 'core.admin', $assetId) !== true)
						{
