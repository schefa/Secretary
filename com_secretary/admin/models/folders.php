...
class SecretaryModelFolders extends \Joomla\CMS\MVC\Model\ListModel
{
	private $app;
	protected $extension;
	protected $business;
	...
	public function getItems()
	{
        ...
        foreach ($items as $item)
		{
            ...
            if (!$item->canChange)
			{
                    $item->canChange = $user->authorise('core.edit.state', 'com_secretary.folder');
            }
    
            if (!$canSee)
			{
                    $canSee = $user->authorise('core.show.other', 'com_secretary.folder.' . $item->id);
            }
            ...
            if (strpos($item->title, 'COM_') !== false)
			{
