...
class SecretaryModelDocument extends \Joomla\CMS\MVC\Model\AdminModel
{
	...
	public function getItem($pk = null)
	{
		if (empty(self::$_item[$pk]) && ($item = parent::getItem($pk)))
		{
			...
			// Always hand the views an array - they index into subject[1..6] and
			// a document without a contact used to leave this null.
			...
			if (empty($item->id))
			{
				if (!empty($item->category->fields) && $catFields = json_decode($item->category->fields))
				{
                    ...
                    foreach ($catFields as $key => $value)
					{
                        ...
                        {
                            if ('pUsage' === $value[3] && empty($item->productUsage))
                            {
                                                    $item->productUsage = $value[2];
                            }
                            else
                            {
                                if ('emailtemplate' === $value[3] && empty($item->message))
                                            {
			...
			$item->document_title = (empty($item->category->alias)) ? \Joomla\CMS\Language\Text::_('COM_SECRETARY_DOCUMENT') : \Joomla\CMS\Language\Text::_($item->category->alias);
	...
	private function _createPDF($data)
	{
		...
		if (!\Joomla\Filesystem\Folder::exists($path))
		{
		...
		{
			\Joomla\Filesystem\File::delete($path . $filename);
		}
