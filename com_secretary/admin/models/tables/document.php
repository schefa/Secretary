...
class SecretaryTableDocument extends \Joomla\CMS\Table\Table
{
	...
	public function bind($array, $ignore = '')
	{
		...
		{
			$actions = \Joomla\CMS\Access\Access::getActionsFromFile(JPATH_ADMINISTRATOR . '/components/com_secretary/access.xml', "/access/section[@name='document']/");
			$default_actions = \Joomla\CMS\Access\Access::getAssetRules('com_secretary.document.' . $array['id'])->getData();
			$array_jaccess = array();
	...
	public function prepareStore(&$data)
	{
		...
		// The deadline field is optional; an empty string is not a valid date for MySQL.
		...
		{
			// Regular line items go through Secretary\Helpers\TaxCalc (the same engine,
			// same rounding, as media/secretary/js/secretary.taxcalc.js) so the stored
			// subtotal/total/taxtotal can never drift from what the live client-side
			// preview showed. Linked sub-document rows (product['id'] > 0) are a separate,
			// older feature that embeds another document's own already-computed
			// subtotal/total directly - they keep their original handling below and are
			// added on top of the regular items' totals; a discount (rabatt) applies only
			// to the regular items, not to embedded sub-document totals.
			$calcItems = array();
			...
			foreach ($data['items'] as $product)
			{
				if (isset($product))
				{
                    ...
                    else
					{
						...
						$itemsRow[$position]['quantity'] = ($product['quantity'] > 0) ? floatval($product['quantity']) : 0;

						$itemsRow[$position]['entity'] = htmlentities($product['entity']);
						$itemsRow[$position]['price'] = ($product['price'] > 0) ? floatval($product['price']) : 0;
						$itemsRow[$position]['total'] = ($product['total'] != 0) ? floatval($product['total']) : 0;
						$itemsRow[$position]['taxRate'] = ($product['total'] != 0) ? floatval($product['taxRate']) : 0;

						if ($product['quantity'] > 0 && $product['total'] == 0)
						{
							$itemsRow[$position]['total'] = (floatval($itemsRow[$position]['quantity']) * floatval($itemsRow[$position]['price']));
                        }

						if (empty($itemsRow[$position]['taxRate']) || $itemsRow[$position]['taxRate'] < 0)
						{
							$itemsRow[$position]['taxRate'] = 0;
                        }

						$calcItems[] = array(
							'total' => $itemsRow[$position]['total'],
							'taxRate' => $itemsRow[$position]['taxRate'],
						);
			...
			$totals = \Secretary\Helpers\TaxCalc::computeDocumentTotals(
				(int) $data['taxtype'],
				$calcItems,
				array('amount' => (float) ($data['rabatt'] ?? 0))
			);

			$data['taxtotal'] = !empty($totals['byRateRaw']) ? json_encode($totals['byRateRaw']) : FALSE;
			$data['subtotal'] = $totals['subtotal'] + array_sum($subtotalRow);
			$data['total'] = $totals['total'] + array_sum($totalRow);
		}

		$data['rabatt'] = !empty($data['rabatt']) ? $data['rabatt'] : 0;
        ...
        if (!empty($data['subject']) && is_array($data['subject']))
		{
            ...
            if (empty($data['subjectid']) && !empty($data['subject'][1]))
			{
				$data['subjectid'] = \Secretary\Helpers\Subjects::addNewSubject($data['subject'], $data['created']);
            }
	...
	public function check()
	{
		...
		// No Contact
		$subject = (array) json_decode($this->subject);
		
        if (strlen($subject[1]) < 1)
		{
			$errTitle = \Joomla\CMS\Language\Text::_('COM_SECRETARY_SUBJECT');
		...
		if (property_exists($this, 'nr'))
		{
            ...
            if (!empty($NrExists))
			{
				$this->setError(\Joomla\CMS\Language\Text::_('COM_SECRETARY_ERROR_DOUBLE_NO'));
		...
		if ($this->rabatt < 0)
		{
			$this->setError(\Joomla\CMS\Language\Text::sprintf('COM_SECRETARY_ERROR_CHECK_THIS', \Joomla\CMS\Language\Text::_('COM_SECRETARY_RABATT')));
		...
		{
			$errTitle = \Joomla\CMS\Language\Text::_('COM_SECRETARY_PAIDAMOUNT');
			$this->setError(\Joomla\CMS\Language\Text::sprintf('COM_SECRETARY_ERROR_CHECK_THIS', $errTitle));
	...
	public function delete($pk = null)
	{
        ...
        if ($result)
		{
			\Secretary\Joomla::getApplication()->enqueueMessage(\Joomla\CMS\Language\Text::_('COM_SECRETARY_DOCUMENT_DELETED_STATUS_UPLOADS'), 'warning');
			...
			$repetition = \Joomla\CMS\Table\Table::getInstance('Repetition', 'SecretaryTable');
