...
class JFormFieldTasks extends \Joomla\CMS\Form\Field\ListField
{
	...
	protected function getOptions()
	{
		...
		for ($i = 0, $n = count($tasks ?? []); $i < $n; $i++)
		{
			...
			{
				$tasks[$i]->text = str_repeat('- ', $tasks[$i]->level) . \Joomla\CMS\Language\Text::_($tasks[$i]->text);
			}
            else
			{
				unset($tasks[$i]);
		...
		array_unshift($tasks, \Joomla\CMS\Language\Text::_('COM_SECRETARY_SELECT_OPTION'));
