...
class SecretaryModelBusiness extends \Joomla\CMS\MVC\Model\AdminModel
{
	...
	public function getItem($pk = null)
	{
		if (empty(self::$_item[$pk]) && ($item = parent::getItem($pk)))
		{
			...
			$item->vatid         = Secretary\Utilities::cleaner($item->vatid, true);
			...
			$selectedCategories  = array('documents' => array(), 'subjects' => array(), 'products' => array());
			// These columns are empty on a business that has not been configured
			// yet; json_decode('') yields the same null it used to yield for null.
