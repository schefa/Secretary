...
class SecretaryModelSubjects extends \Joomla\CMS\MVC\Model\ListModel
{
    ...
    public function getItems()
    {
        ...
        foreach ($items as $item)
		{
            ...
            if (
                isset($item->created_by) && ($user->id == $item->created_by && $user->authorise('core.edit.own', 'com_secretary.subject'))
            ...
            if (!$item->canChange)
            {
            	$item->canChange = $user->authorise('core.edit.state', 'com_secretary.subject');
            }
            
            if (!$canSee)
            {
            	$canSee = $user->authorise('core.show.other', 'com_secretary.subject.' . $item->id);
            }
            ...
            // firstname/lastname/street/zip/location/phone/email are stored in cleaner()'s
            // HTML-entity-encoded form; decode them back here (once, centrally for the whole
            // list) rather than leaving every template that displays these fields to rely on
            // the browser happening to decode entities in text/attribute contexts on its own.
