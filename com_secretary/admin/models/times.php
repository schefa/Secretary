...
class SecretaryModelTimes extends \Joomla\CMS\MVC\Model\ListModel
{
	...
	public function getItems()
	{
		...
		if (!empty($items))
		{
            ...
            foreach ($items as $x => $item)
			{
                ...
                if (!$item->canChange)
				{
                        $item->canChange = $user->authorise('core.edit.state', 'com_secretary.time');
                }
    
                if (!$canSee)
				{
                        $canSee = $user->authorise('core.show.other', 'com_secretary.time.' . $item->time_id);
                }
				...
				{
                    $item->additionalTitle = '<a href="index.php?option=com_secretary&task=time.add&extension=' . $item->extension . '&pid=' . $item->id . '" class="secretary-time-title-add hasTooltip btn btn-small" title="' . \Joomla\CMS\Language\Text::_('COM_SECRETARY_PROJECT_NEW_TASK') . '"><i class="fa fa-plus"></i></a>';
                    $item->timeColor = '';
                }
                elseif ($item->extension == 'projects')
				{
                    ...
                    if ($user->authorise('core.create', 'com_secretary.time'))
                            {
                        $item->additionalTitle .= '<a href="index.php?option=com_secretary&task=time.add&extension=tasks&pid=' . $item->time_id . '" class="secretary-time-title-add hasTooltip btn btn-small" title="' . \Joomla\CMS\Language\Text::_('COM_SECRETARY_PROJECT_NEW_TASK') . '"><i class="fa fa-plus"></i></a>';
                    }
	...
	public function getWeekItems($week, $year)
	{
        ...
        if (!empty($events))
		{
			...
			foreach ($events as $eventKey => $event)
			{
                ...
                if (!$canSee)
				{
					$canSee = $user->authorise('core.show.other', 'com_secretary.time.' . $event->id);
				}
				...
				// Precalculation
				// startDate/endDate are nullable columns; without a fallback, an event
				// with a NULL date crashes the "+" arithmetic further down on the
				// resulting empty time string.
				$startDate = explode(" ", $event->startDate ?: $event->endDate ?: '1970-01-01 00:00:00');
				$endDate = explode(" ", $event->endDate ?: $event->startDate ?: '1970-01-01 00:00:00');
