...
class SecretaryModelTime extends \Joomla\CMS\MVC\Model\AdminModel
{
	...
	{
		return \Joomla\CMS\Table\Table::getInstance($this->tableName, $prefix, $config);
	}
	...
	protected function loadFormData()
	{
        ...
        if (empty($item))
		{
            ...
            if (empty($item->catid) && ($this->catid > 0))
			{
			...
			if (isset($item->calctime))
			{
				$item->calctime = $item->calctime / 3600;
            }
		}
	...
	public function getItem($pk = null)
	{
		if (empty(self::$_item[$pk]) && ($item = parent::getItem($pk)))
		{
			...
			if (empty($item->location_id) && !empty($this->locationid))
			{
				$item->location_id = $this->locationid;
			}
	...
	public function save($data)
	{
		...
		try
		{
			...
			{
				$errTitle = \Joomla\CMS\Language\Text::_('COM_SECRETARY_TITLE');
			...
			if (isset($data['calctime']))
			{
				$data['calctime'] = $data['calctime'] * 3600;
			}
	...
	public function getProjects($id = NULL)
	{
		...
		{
			if (
				$user->authorise('core.show', 'com_secretary.time.' . $project->id)
				|| $user->authorise('core.show.other', 'com_secretary.time.' . $project->id)
			)
			{
				$result['items'][]	= \Joomla\CMS\HTML\HTMLHelper::_('select.option',	$project->id, \Joomla\CMS\Language\Text::_($project->title));
            }
		}
