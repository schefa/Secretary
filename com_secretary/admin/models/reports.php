...
class SecretaryModelReports extends \Joomla\CMS\MVC\Model\ItemModel
{

	/**
	 * Joomla 6 declares ItemModelInterface::getItem() abstract. This model is a
	 * report aggregator and has no single item, so it satisfies the contract
	 * without pretending to load one.
	 *
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		{
			$this->setState('reports_start_date', $end_date);
		}

		if (empty($start_date))
		{
			$this->setState('reports_start_date', date("Y") . '-01-01');
		}
		
        if (empty($end_date))
		{
			$this->setState('reports_end_date', date("Y-m-d"));
		}
	...
	public function getProductsGrowth(array $business)
	{
		...
		try
		{
            ...
            for ($x = 0; $x < count($items ?? []); $x++)
			{
				...
				{
					$tmp['growth']['series'][$x] = round($items[$x]->total ?? 0, 2);
                }
	...
	public function getContactsGrowth(array $business)
	{
        ...
        switch ($zeitraum)
		{
			default:
            case 1:
				$query->select($weekStr . ' AS zeitraum');
				break;
			case 2:
				$query->select($monthStr . ' AS zeitraum');
				break;
			case 3:
				$query->select($yearStr . ' AS zeitraum');
				break;
		}
		...
		try
		{
            ...
            for ($x = 0; $x < count($items ?? []); $x++)
			{
				...
				if (!isset($tmp['categories']['series'][$items[$x]->catid]))
				{
                    ...
                    if (strlen((string) $items[$x]->category) <= 1)
					{
						$items[$x]->category = 'COM_SECRETARY_NONE';
					}
					$tmp['categories']['labels'][$items[$x]->catid] = \Joomla\CMS\Language\Text::_($items[$x]->category);
                ...
                if (!isset($tmp['location']['series'][$zip]))
				{
                    ...
                    if (strlen($zip) <= 2)
					{
						$tmp['location']['labels'][$zip] = \Joomla\CMS\Language\Text::_('COM_SECRETARY_NONE');
                    }
                    else
					{
                ...
                if (!isset($tmp['genders']['series'][$items[$x]->gender]))
				{
                    ...
                    $tmp['genders']['labels'][$items[$x]->gender] = (strlen($title) > 0) ? $title : \Joomla\CMS\Language\Text::_('COM_SECRETARY_NONE');
                ...
                elseif ($zeitraum == 3)
				{
                            ...
                            $tmp['growth']['labels'][$zeitraumKey][1] = \Joomla\CMS\Language\Text::_('COM_SECRETARY_YEAR') . ': ' . $items[$x]->zeitraum;
                }
                else
				{
                            $weekHTML = array();
				...
				$statusTitle = \Joomla\CMS\Language\Text::_($items[$x]->statusTitle);
	...
	public function getStats(array $business, $guv = NULL)
	{
		...
		if (!empty($guv))
		{
			try
			{
				...
				\Joomla\Utilities\ArrayHelper::toInteger($guv);
				$guv = implode(',', $guv);
	...
	private function remakeItem(&$newItems, $i, $type, $item)
	{
        ...
        if (!isset($newItems['data'][$zeitraum][$type][$currency][$item[$i]->state]))
		{
			...
			$newItems['data'][$zeitraum][$type][$currency][$item[$i]->state][0] = \Joomla\CMS\Language\Text::_($item[$i]->status);
