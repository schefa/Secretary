...
class SecretaryModelDocuments extends \Joomla\CMS\MVC\Model\ListModel
{
    ...
    public function getListQuery()
    {
        ...
        elseif (
            $this->user->authorise('core.show', 'com_secretary.document')
    ...
    public function getItems()
    {
        ...
        foreach (self::$items as $item)
		{
            ...
            if (!$canSee)
            {
            	$canSee = $this->user->authorise('core.show.other', 'com_secretary.document.' . $item->id);
            }
            ...
            // Folder title
            $item->category_title = (empty($item->category_title)) ? \Joomla\CMS\Language\Text::_('COM_SECRETARY_DOCUMENT') : \Joomla\CMS\Language\Text::_($item->category_title);
    ...
    public function getSummary()
    {
        ...
        foreach ($items as $item)
		{
            ...
            if (!$item->canCheckin)
            {
            	$item->canCheckin = $this->user->authorise('core.admin', 'com_secretary');
            }
            
            if (!$item->canChange)
            {
            	$item->canChange = $this->user->authorise('core.edit.state', 'com_secretary.document');
            }
        ...
        if (!empty($this->catId) && is_numeric($this->catId))
		{
            ...
            if (!empty($category))
			{
                $this->items_report['alias'] = \Joomla\CMS\Language\Text::_($category->alias);
                $this->items_report['title'] = \Joomla\CMS\Language\Text::_($category->title);
            }
        }
        else
		{
            $this->items_report['alias'] = \Joomla\CMS\Language\Text::_('COM_SECRETARY_DOCUMENT');
            $this->items_report['title'] = \Joomla\CMS\Language\Text::_('COM_SECRETARY_DOCUMENTS');
        }
