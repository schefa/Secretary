...
class SecretaryModelItem extends \Joomla\CMS\MVC\Model\AdminModel
{
	protected $app;
	protected $business;
	protected $extension;
	...
	public function getItem($pk = null)
	{
		...
		if (empty($this->_item) && ($item = parent::getItem($pk)))
		{
			...
			{
				case ('fields'):
					$item->title = \Joomla\CMS\Language\Text::_($item->title);
                    ...
                    if (!empty($item->values) && ($values = json_decode($item->values)))
					{
						if (is_array($values))
						{
							...
							{
								$newValues[] = \Joomla\CMS\Language\Text::_($value);
							}
				...
				case ('status'):
					$item->title = \Joomla\CMS\Language\Text::_($item->title);
					$item->description = \Joomla\CMS\Language\Text::_($item->description);
		...
		try
		{
			...
			switch ($this->extension)
			{
				...
				case 'fields':
					...
					if (empty($data['title']))
					{
						$this->setError(\Joomla\CMS\Language\Text::sprintf('COM_SECRETARY_ERROR_CHECK_THIS', 'No Title'));
				...
				case 'uploads':
					...
					if ($user->authorise('core.upload', 'com_secretary'))
					{
						...
						{
							$this->setError(\Joomla\CMS\Language\Text::_('COM_SECRETARY_NO_FILE_SELECTED'));
					...
					{
						$this->setError(\Joomla\CMS\Language\Text::_('JERROR_ALERTNOAUTHOR'));
