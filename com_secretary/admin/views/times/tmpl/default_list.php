...
if (empty($this->items))
{
	echo '<div class="alert alert-no-items">'. \Joomla\CMS\Language\Text::_('COM_SECRETARY_NO_MATCHING_RESULTS') .'</div>';
...
else
{
    ...
    // Berechnung der Startposition der grafischen Hervorhebung für Heute und Alle Tage
   	if ($this->intervall > 0)
   	{
        $lineToday =  $horzLines *  ( ($this->today - $this->vorspannSecs) / $this->intervall ) ; }
	else
	{
        $lineToday = 1; }
...
<div class="time-calender fullwidth">
    ...
    <div id="documentsList" style="overflow:hidden;">
        ...
        <div class="secretary-row-inner clearfix thead">
            ...
            <div class="order nowrap center hidden-phone"><?php if (true === $this->canDo->get('core.edit'))
            {
                ?><a onclick="Joomla.submitbutton('times.saveOrder')"><i class="fa fa-save"></i></a><?php } ?>&nbsp;</div>
            
			<div class="progress-td">&nbsp;</div>
                
            <div class="title">
                <?php echo \Joomla\CMS\HTML\HTMLHelper::_('grid.sort',  'COM_SECRETARY_TITLE', 'time.title', $listDirn, $listOrder); ?> / <?php echo \Joomla\CMS\HTML\HTMLHelper::_('grid.sort',  'COM_SECRETARY_TIMES_STARTDATE', 'startDate', $listDirn, $listOrder); ?>
            </div>
