...
<form action="<?php echo \Joomla\CMS\Router\Route::_('index.php?option=com_secretary&view=items'. $extension); ?>" method="post" name="adminForm" id="adminForm">
...
{
    ?>
    <div class="alert alert-message"><?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_ENTITIES_NOENTITYSELECT'); ?></div>
<?php } ?>
        ...
        <?php if ( $this->extension == 'status' || $this->extension == 'fields')
        {
            ...
            <div class="pull-right select-arrow">
                <select id="filter_section" onchange="this.form.submit()" name="filter_section"> 
                    <?php echo \Joomla\CMS\HTML\HTMLHelper::_('select.options', $this->sections, 'id', 'title', $this->section, true);?>
                </select>
            </div>
        <?php } ?>
    </div>
