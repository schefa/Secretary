...
<form action="<?php echo \Joomla\CMS\Router\Route::_('index.php?option=com_secretary&view=products'.$category); ?>" method="post" name="adminForm" id="adminForm">
    ...
    <?php if ($this->canDo->get('core.show'))
    {
		...
		<div class="row-fluid clearfix">
			<div class="pull-left">
        		<h2 class="documents-title">
                    <span class="documents-title-first"><?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_PRODUCTS'); ?></span>
                    <span class="documents-title-second">
                    <a href="<?php echo \Joomla\CMS\Router\Route::_('index.php?option=com_secretary&view=folders&extension=products'); ?>"><?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_CATEGORIES');?></a>
                    </span>
                    <span class="documents-title-second">
                    <a href="<?php echo \Joomla\CMS\Router\Route::_('index.php?option=com_secretary&view=locations&extension=products'); ?>"><?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_LOCATIONS_PRODUCTS');?></a>
                    </span>
                </h2>
			</div>
			...
			<div class="pull-right margin-right">
            	<div class="select-arrow select-arrow-white select-small">
                    <select id="filter_year" onchange="this.form.submit()" name="filter_year"> 
                        <?php echo \Joomla\CMS\HTML\HTMLHelper::_('select.options', $this->years, 'value', 'text', $this->state->get('filter.year'), true);?>
                    </select>
                </div>
			</div>
        </div>
		...
		<div class="row-fluid secretary-toolbar clearfix">
            ...
            <div class="pull-right select-arrow-control">
                ...
                <span class="select-label"><?php echo \Joomla\CMS\Language\Text::sprintf('COM_SECRETARY_CREATE_THIS', $document); ?></span>
                ...
                <div id="add_document" class="btn btn-newentry"><?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_NEW'); ?></div>
                <script>
                (function($){
		...
		<div class="secretary-products-stockwatch">
            ...
            foreach ($this->items as $i => $item)
            {
                if ($item->quantityMin > 0)
                {
                    ...
                    if ($need > 0)
                    {
                        echo '<div class="products-needed">'.\Joomla\CMS\Language\Text::sprintf('COM_SECRETARY_PRODUCTS_WATCH_ARE_NEEDED','<strong>'.$need .' '.$item->entity.'</strong>', '<strong>'.$item->title.'</strong>').'</div>';
                    }
                }
                ...
                if ($item->quantityMax > 0)
                {
                    ...
                    if ($seed > 0)
                    {
                        echo '<div class="products-toomuch">'.\Joomla\CMS\Language\Text::sprintf('COM_SECRETARY_PRODUCTS_WATCH_TOO_MUCH','<strong>'.$seed .' '.$item->entity.' '.$item->title.'</strong>', number_format($item->quantityMax)).'</div>';
                    }
                }
            }
            ?>
		...
		<div class="chk_items_container" style="display:none;">
            ...
            ?></div>
            <button type="button" class="btn btn-newentry" onclick="Joomla.submitbutton('products.applyColumns')"><?php echo \Joomla\CMS\Language\Text::_("COM_SECRETARY_SAVE")?></button>
        </div>
		...
		<table class="table table-hover" id="documentsList">
			<thead>
				...
				<tr>
                	<th colspan="<?php echo (3 + $thPos['endOther'])?>">
                		<a href="#" class="custom-columns-btn"><?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_COLUMNS_ADAPT')?></a>
                	</th>
                	<?php if (false !== ($thPos['startEingang']))
                	{
                        ?>
                	<th colspan="<?php echo $thPos['endEingang'] - $thPos['startEingang'] ?>" class='center products-list-summary'><?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_EINGANG');?></th>
                	<?php } ?>
                            <?php 
                            if (false !== ($thPos['startAusgang']))
                            {
                                ?>
                	<th colspan="<?php echo $thPos['endAusgang'] - $thPos['startAusgang'] ?>" class='center products-list-summary'><?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_AUSGANG');?></th>
                            <?php } ?>
                	<th colspan="2" class='center products-list-summary'><?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_VALUE_OF_GOODS');?></th>
				</tr>
				...
				<tr>
                    ...
                    <th class='left'>
                            <?php echo \Joomla\CMS\HTML\HTMLHelper::_('grid.sort',  'COM_SECRETARY_PRODUCT_TITLE', 'a.title', $listDirn, $listOrder); ?>
                    </th>
                    	...
                    	<th class='nowrap <?php echo  $css ?>' >
                                <?php echo \Joomla\CMS\HTML\HTMLHelper::_('grid.sort',  'COM_SECRETARY_PRODUCT_'.$key, 'a.'.$key, $listDirn, $listOrder); ?>
                    	</th>
                                <?php   
                                        $z++;
                            }
                    ...
                    <th class='right products-list-summary'>
                            <?php echo \Joomla\CMS\HTML\HTMLHelper::_('grid.sort',  'COM_SECRETARY_QUANTITY', 'a.quantity', $listDirn, $listOrder); ?>
                    </th>
                    <th class='right products-list-total-rest'>
                    <?php echo \Joomla\CMS\HTML\HTMLHelper::_('grid.sort',  'COM_SECRETARY_VALUE', 'a.total', $listDirn, $listOrder); ?>
                    </th>
			...
			<tbody>
				...
				<tr class="row<?php echo $i % 2; ?>">
                    ...
                    <td>
                    <a class="hasTooltip" data-original-title="<?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_SHOW'); ?>" title="<?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_SHOW'); ?>"  href="<?php echo \Joomla\CMS\Router\Route::_('index.php?option=com_secretary&view=product&id='.(int) $item->id); ?>"><i class="fa fa-newspaper-o"></i></a>
                    ...
                    <td>
                        ...
                        {
                            ?>
                            <?php echo \Joomla\CMS\HTML\HTMLHelper::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'products.', $item->canCheckin); ?>
                        <?php } ?>
                        <?php 
                        if ($item->canEdit)
                        {
                            ?>
                            <a class="hasTooltip" data-original-title="<?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_CLICK_TO_EDIT'); ?>"  href="<?php echo \Joomla\CMS\Router\Route::_('index.php?option=com_secretary&task=product.edit&id='.(int) $item->id); ?>">
                    ...
                    <td class="text-right products-list-total-rest nowrap">
                        <?php echo Secretary\Utilities\Number::transformAmount($bestandsWert,'get', $currency, \Joomla\CMS\Language\Text::_('COM_SECRETARY_VALUE_WHAT_VK')); ?>
                    </td>
        ...
        <?php }?>
			...
			<div class="margin-top">
                	...
                	<div class="pull-left"><?php echo $this->pagination->getListFooter(); ?></div>
           	 		<div class="pull-left limit-box clearfix"><span class="pagination-filter-text"><?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_LIMIT');?></span><?php echo $this->pagination->getLimitBox(); ?></div>
           	 		<div class="pull-left clearfix">
                    <select name="sortTable" id="sortTable" class="" onchange="Joomla.orderTable()"><option value=""><?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_SORT_BY');?></option><?php echo \Joomla\CMS\HTML\HTMLHelper::_('select.options', $this->getSortFields(), 'value', 'text', $listOrder);?></select>
                    </div> 
                	<div class="pull-left">
