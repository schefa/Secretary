...
$reportCurrency        = $this->state->get('filter.docscurrency');
$hasMultipleCurrencies  = !$reportCurrency && count($this->documents['currency']) > 1;

$currOptions[] = \Joomla\CMS\HTML\HTMLHelper::_('select.option', 0, \Joomla\CMS\Language\Text::sprintf('COM_SECRETARY_FILTER_SELECT_LABEL_ALL',  \Joomla\CMS\Language\Text::_('COM_SECRETARY_CURRENCIES')) );
...
{
    $documentsCharts['legend'][] = '<span class="legend-colorbox legend-income status-'.$state->class.'"></span><span class="legend-colorbox legend-costs status-'.$state->class.'"></span><span class="legend-text">'. \Joomla\CMS\Language\Text::_($state->text) .'</span>';
}
?>
...
<div class="btn-toolbar-charts fullwidth">
    <div class="item-stats">
        <div class="subitem">
            <label class="control-label"><?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_ERGEBNIS'); ?></label>
            <div class="controls">
<?php 
foreach ($this->documents_total as $cur => $values)
{
    ...
    echo Secretary\Utilities\Number::getNumberFormat($profit,$cur); 
    echo '</strong><br>'; 
}
?>
            </div>
    ...
    <div class="item-stats">
        ...
        <div class="subitem">
            <label class="control-label"><?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_EINNAHMEN'); ?></label>
        ...
        <div class="subitem">
            <label class="control-label"><?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_AUSGABEN'); ?></label>
    ...
    <div class="item-stats">
        ...
        <div class="subitem">
            <label class="control-label"><?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_UMSATZSTEUER'); ?></label>
            <div class="controls">
            <?php 
            foreach ($this->documents_total as $cur => $values)
            {
        ...
        <div class="subitem">
            <label class="control-label"><?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_VORSTEUER'); ?></label>
            <div class="controls">
            <?php 
            foreach ($this->documents_total as $cur => $values)
            {
    ...
    <div class="item-stats pull-right">
    	<div class="btn-group">
            ...
            <select class="form-control"  name="filter_docs_state">
                <option value="0"><?php echo \Joomla\CMS\Language\Text::sprintf('COM_SECRETARY_FILTER_SELECT_LABEL_ALL',  \Joomla\CMS\Language\Text::_('COM_SECRETARY_STATUS')); ?></option>
                <?php echo \Joomla\CMS\HTML\HTMLHelper::_('select.options', $this->docsStates, 'value', 'text', $this->state->get('filter.docsstate'), true);?>
            </select> 
    	...
    	<div class="btn-group">
            <div class="pull-right select-arrow select-arrow-white">
            	<select class="form-control" name="filter_docs_currency">
                    <?php echo \Joomla\CMS\HTML\HTMLHelper::_('select.options', $currOptions, 'value', 'text',  $this->state->get('filter.docscurrency') );?>
                </select>
    		</div>
    	</div>
...
<div class="row-fluid fullwidth">
    <div class="col-md-12">
    <?php if ($hasMultipleCurrencies)
    {
        ?>
        <h3><?php echo \Joomla\CMS\Language\Text::_("COM_SECRETARY_CHARTS_INCOME_AND_COSTS");?></h3>
        <div class="alert alert-message"><?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_REPORTS_SELECT_CURRENCY'); ?></div>
    <?php }
    else
    {
        $currencyIncomeCosts = $reportCurrency ?: $this->business['currencySymbol'];
        ?>
        <h3><?php echo \Joomla\CMS\Language\Text::_("COM_SECRETARY_CHARTS_INCOME_AND_COSTS") .' '. $currencyIncomeCosts;?></h3>
...
<table class="table table-striped" id="overviewList">
    <thead>
        ...
        <?php if ($filterZeitraum != 3)
        {
            ?>
        <th class='left'>
            <?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_YEAR'); ?>
        </th>
        <?php }?>
        
        <th class="left table-right-border">
        <?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_DATE'); ?>
        </th>
        <th class='left'>
        ...
        <?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_DIFFERENCE'); ?>
        </th>
    ...
    foreach ($this->documents['data'] as $i => $item)
    {
        ...
        elseif ($filterZeitraum == 3)
        {
            ...
            $documentsCharts['labels'][$i][1] = \Joomla\CMS\Language\Text::_('COM_SECRETARY_YEAR') .': '. $i;
        }
        else
        {
            $weekHTML = array();
        ...
        {
            $es = Secretary\HTML::_('reports.documentsItem', $i, $item, 'income', $documentsCharts);
        ...
        <td class="nowrap">
            ...
            {
                $es = Secretary\HTML::_('reports.documentsItem', $i, $item, 'costs', $documentsCharts);
                $documentsCharts = $es['data'];
                echo $es['html'];
            }
            ?>
