...
<form action="<?php echo \Joomla\CMS\Router\Route::_('index.php?option=com_secretary&view=subjects'. $category); ?>" method="post" name="adminForm" id="adminForm">
...
<div class="fullwidth">
    ...
    <div class="secretary-main-container-left">
        ...
        <div class="fullwidth">
            <div class="pull-left">
                <h2 class="documents-title">
                    ...
                    <?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_SUBJECTS'); ?></span>
            ...
            <div class="pull-right">
                <div class="secretary-search btn-group">
                    <input type="text" class="form-control filter_search_subject" name="filter_search" id="filter_search" placeholder="<?php echo \Joomla\CMS\Language\Text::_('JSEARCH_FILTER'); ?>" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" title="<?php echo \Joomla\CMS\Language\Text::_('JSEARCH_FILTER'); ?>" />
		...
		<div class="chk_items_container" style="display:none;">
            ...
            ?></div>
            <button type="button" class="btn btn-newentry" onclick="Joomla.submitbutton('subjects.applyColumns')"><?php echo \Joomla\CMS\Language\Text::_("COM_SECRETARY_SAVE")?></button>
        </div>
        ...
        <table class="table table-hover" id="documentsList">
            <thead>
                <tr>
                	...
                	<span class="custom-columns-btn btn-link"><?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_COLUMNS_ADAPT')?></span>    
                	</th>
                ...
                <tr>
                    ...
                    <th></th>
                    <th class='left'>
                        <?php echo \Joomla\CMS\HTML\HTMLHelper::_('grid.sort','COM_SECRETARY_SUBJECT_NAME','a.lastname',$listDirn,$listOrder); ?>
                    </th>
                    <?php 
                    foreach ($this->acceptedColumns as $key => $value)
                    {
                    	...
                    	<th class='nowrap <?php echo  $css ?>' >
                    		<?php echo \Joomla\CMS\HTML\HTMLHelper::_('grid.sort',  'COM_SECRETARY_'.$key, 'a.'.$key, $listDirn, $listOrder); ?>
                    	</th>
                        <?php   
                    }
                    ?>
            ...
            <tbody>
                ...
                <tr class="row<?php echo $i % 2; ?>">
                    ...
                    <td>
                    <a class="hasTooltip" data-original-title="<?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_SHOW'); ?>" title="<?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_SHOW'); ?>"  href="<?php echo \Joomla\CMS\Router\Route::_('index.php?option=com_secretary&view=subject&id='.(int) $item->id); ?>"><i class="fa fa-newspaper-o"></i></a>
                    ...
                    <td>
                        ...
                        {
                            ?>
                            <?php echo \Joomla\CMS\HTML\HTMLHelper::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'subjects.', $item->canCheckin); ?>
                    <?php } ?>
                    
                        <?php if ($item->canEdit && !$item->checked_out)
                        {
                            ?>
                        <a class="hasTooltip" data-original-title="<?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_CLICK_TO_EDIT'); ?>" title="<?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_CLICK_TO_EDIT'); ?>"  href="<?php echo \Joomla\CMS\Router\Route::_('index.php?option=com_secretary&task=subject.edit&id='.(int) $item->id); ?>">
                    ...
                    <?php } ?>
                    	...
                    	<td class="<?php echo (isset($this->items[0]->$key) && is_numeric($this->items[0]->$key)) ? 'right' : 'left' ?>">
                            ...
                            if (isset($item->$key))
                            {
                                ...
                                {
                                    echo ($key === 'category') ? \Joomla\CMS\Language\Text::_($item->$key) : $item->$key;
                                }
                            }
                            ?>
                    	</td>
        ...
        <ul class="secretary-list-group">
            <?php if (!empty($this->zip))
            {
                ...
                <li class="list-group-item">
                    ...
                    <a href="<?php echo Secretary\Route::create( 'subjects', array( 'catid'=> $this->catid )); ?>"><?php echo \Joomla\CMS\Language\Text::_('COM_SECRETARY_RESET_FILTER'); ?></a>
                </li>
            <?php } ?>
            <?php 
            foreach ($this->contact_locations AS $contact_locations)
            {
                ?>
                ...
                {
                        echo \Joomla\CMS\Language\Text::_('diverse');
                }
                ?>
                	</span>
