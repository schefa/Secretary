...
class SecretaryViewSubjects extends \Joomla\CMS\MVC\View\HtmlView
{
	protected $items;
	...
	public function display($tpl = null)
	{
		...
		$this->folders		= \Joomla\CMS\Form\FormHelper::loadFieldType('Categories', false)->getCategories( $this->view );
	...
	protected function addToolbar()
	{
		...
		{
			$addEventText = \Joomla\CMS\Language\Text::_('COM_SECRETARY_SUBJECT');
			$html[] = Secretary\Navigation::ToolbarItem('subject.add', \Joomla\CMS\Language\Text::sprintf('COM_SECRETARY_NEW_ENTRY_TOOLBAR',$addEventText), false, 'newentry' );
		
			if ($this->canDo->get('core.edit'))
			{
				$html[] = '<button type="button" class="btn btn-default hasTooltip" onclick="Joomla.submitbutton(\'subjects.importUsers\')" title="'.\Joomla\CMS\Language\Text::_("COM_SECRETARY_SUBJECTS_IMPORT_DESC").'">
		<span class="fa fa-arrow-circle-o-down"></span>'. \Joomla\CMS\Language\Text::_("COM_SECRETARY_SUBJECTS_IMPORT") . '</button>';
			}
			 
			if ($this->canDo->get('core.edit'))
			{
			    $title = \Joomla\CMS\Language\Text::sprintf('COM_SECRETARY_CREATE_THIS', \Joomla\CMS\Language\Text::_('COM_SECRETARY_DOCUMENT'));
		...
		if (isset($this->items[0]) && $this->canDo->get('core.edit') && $this->canDo->get('core.edit.state'))
		{
			...
			$html[] = "<button data-joomla-dialog='{\"popupType\": \"inline\", \"src\": \"#secretary-batch-dialog\"}' type=\"button\" class=\"btn btn-small\"><span class=\"fa fa-database\" title=\"". $title ."\"></span>". $title ."</button>";
		}
	...
	{
	    $fields = array(
	        'a.firstname' => \Joomla\CMS\Language\Text::_('COM_SECRETARY_FIRSTNAME'),
	        'a.lastname' => \Joomla\CMS\Language\Text::_('COM_SECRETARY_NACHNAME'),
	    );
	    
        foreach ($this->acceptedColumns as $key => $t)
		{
	        $fields['a.'.$key] = \Joomla\CMS\Language\Text::_('COM_SECRETARY_'.$key);
	    } 
