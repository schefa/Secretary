...
echo '<h3 class="title">'. \Joomla\CMS\Language\Text::_('COM_SECRETARY_HISTORY') .'</h3>';
...
if (!empty($histories))
{
    ...
    echo '<tr>';
    echo '<td>'.'</td>';
    echo '<td>'.\Joomla\CMS\Language\Text::_('COM_SECRETARY_DOCUMENT').'</td>';
    echo '<td>'.\Joomla\CMS\Language\Text::_('COM_SECRETARY_QUANTITY').'</td>';
    echo '<td>'.\Joomla\CMS\Language\Text::_('COM_SECRETARY_PRICE').'</td>';
    echo '<td>'.\Joomla\CMS\Language\Text::_('COM_SECRETARY_TOTAL').'</td>';
    echo '</tr>';
    ...
    foreach ($histories AS $key => $history)
    {
        foreach ($history AS $timestamp => $values)
        {
            ...
            if (!is_null($document->category))
                {
                echo \Joomla\CMS\Language\Text::_($document->category->alias);
            }
                echo '</td>';
                echo '<td>'. $values[1] .'</td>';
