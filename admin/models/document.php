...
class SecretaryModelDocument extends JModelAdmin
{
	...
	protected function loadFormData()
	{
		...
		if (empty($data)) {
			...
			if(!isset($data->currency)) $data->currency	= $this->business['currency'];
	...
	public function getItem($pk = null)
	{
		if(empty(self::$_item[$pk]) && ($item = parent::getItem($pk)))
		{
			...
			if($taxtotals = json_decode($item->taxtotal)) {
				$taxtotal = array();
				if(is_numeric($taxtotals)) $taxtotals = array($taxtotals);
				foreach($taxtotals as $tax)	$taxtotal[] = round($tax,2);
			...
			$item->rabatt = ($item->rabatt == 0) ? NULL : $item->rabatt;
			...
			// Contact
			if(count($this->subjectIds) == 1) $this->subject = $this->subjectIds[0];
			...
			if(empty($item->createdEntry)) $item->createdEntry = time(); 

			// Data fields	
			if(!empty($item->fields)) 
			{
				$fields	= json_decode($item->fields, true);
				if(isset($fields['pUsage'])) { $item->productUsage = $fields['pUsage']; unset($fields['pUsage']); }
				if(isset($fields['message'])) { 
					$item->message = $fields['message'];
					unset($fields['message']);
				}
				if(isset($fields['repetition'])) { $item->repetition = $fields['repetition']; unset($fields['repetition']); }
			...
			} elseif($item->accounting_id < 1) {
			    // Accounting pre-defined fields taken from category
				if(!empty($item->category->fields) && $catFields = json_decode($item->category->fields )) {
				    $item->accounting = array();
					$searchArray = array('docsSoll','docsHaben','docsSollTax','docsHabenTax');
					foreach($catFields AS $key => $value) {
						if(!empty(array_intersect($searchArray, $value))) {
						    $title = ($value[3] == 'docsSollTax' || $value[3] == 'docsHabenTax') ? ''.$value[1].'' : FALSE;
							$item->accounting[$value[3]][$title] = (int) $value[2];
						}
					}
				}
			}
			if(empty($item->template)) $item->template = 0; 
			if(!empty($this->productUsage)) { $item->productUsage = $this->productUsage; }
			...
			if(!isset($item->productUsage)) { $item->productUsage = 0; }
			...
			if($item->deadline == '0000-00-00') $item->deadline = NULL;
			...
			if(!isset($item->message['template'])) $item->message['template'] = 0;
