...
class SecretaryControllerTime extends JControllerForm
{
	...
	protected function getRedirectToItemAppend($recordId = null, $urlVar = 'id')
	{
		...
		if($this->extension == 'tasks' && !empty($this->pid)) $append .= '&pid=' . $this->pid;
		if(!empty($this->catid)) $append .= '&catid=' . $this->catid;
		if(!empty($this->locationid)) $append .= '&location_id=' . $this->locationid;
		return $append;
	}
	
	protected function getRedirectToListAppend()
	{
		if($this->extension == 'tasks') $this->extension = 'projects';
		
		$append = parent::getRedirectToListAppend();
		$append .= '&section=' . $this->section;
		if(!empty($this->catid)) $append .= '&catid=' . $this->catid;
		if($this->extension == 'task') $this->extension = 'projects';
		$append .= '&extension=' . $this->extension;
		return $append;
	}
